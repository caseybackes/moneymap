...
export function App() {
      ...
      try {
        ...
        try {
          ...
          if (syncResult.pendingConnections > 0) {
            setStartupSyncWarning("Plaid has not finished loading transaction history. Your saved balances are available. Sync again later.");
          } else {
            setStartupSyncMessage("Connected accounts are up to date.");
          }
        } catch (reason) {
          if (cancelled || epoch !== storeEpoch.current) return;
          setStartupSyncWarning(`Account refresh failed. Showing the last saved data. ${String(reason)}`);
        }
  ...
  async function syncConnectedAccounts() { setSyncingAccounts(true); setError(null); setSyncMessage("Refreshing connected accounts…"); try { const result = await synchronizePlaidHistory((attempt, maxAttempts) => setSyncMessage(`Loading transaction history… retry ${attempt} of ${maxAttempts}`)); setSyncMessage(result.pendingConnections > 0 ? "Plaid has not finished loading transaction history. Sync again later." : result.changed === 0 ? "Up to date." : `Synced ${result.changed} transaction change${result.changed === 1 ? "" : "s"}.`); refresh(); if (view === "ledger" || view === "calendar") void invoke<LedgerData>("ledger_data").then(setLedger); } catch (reason) { setError(String(reason)); } finally { setSyncingAccounts(false); } }
  ...
  return <main className="app-shell">
    ...
    <section className="page">
      ...
      {startupSyncing ? <div className="startup-sync" role="status" aria-live="polite"><span className="sync-spinner" /><span><strong>{startupSyncMessage}</strong><small>You can keep using Money Map.</small></span></div> : null}
...
function PlaidLinkButton({ sandboxEnabled, onImported, compact = false }: { sandboxEnabled: boolean; onImported: () => void; compact?: boolean }) {
  ...
  return <div className="connect-card sandbox-link-card"><span>+</span><strong>{loading ? "Preparing connection…" : sandboxEnabled ? "Connect Sandbox account" : "Connect new account"}</strong><small>{sandboxEnabled ? "Sandbox connection · no Trial slot" : "Connect a financial institution"}</small><button onClick={() => void begin()} disabled={loading}>{loading ? "Working…" : "Connect new account"}</button>{message ? <small className="form-error">{message}</small> : null}</div>;
}
