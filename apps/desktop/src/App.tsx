...
export function App() {
  ...
  return <main className="app-shell">
    ...
    <section className="page">
      ...
      {view === "accounts" ? <Accounts accounts={dashboard?.accounts ?? []} connections={connections} sandboxEnabled={sandboxEnabled} syncMessage={syncMessage} onAdd={() => setDialog("account")} onSync={() => void syncConnectedAccounts()} onDisconnect={(connection) => void disconnectConnectedAccount(connection)} syncing={syncingAccounts} /> : null}
...
function Accounts({ accounts, connections, sandboxEnabled, syncMessage, onAdd, onSync, onDisconnect, syncing }: { accounts: Account[]; connections: ConnectedInstitution[]; sandboxEnabled: boolean; syncMessage: string | null; onAdd: () => void; onSync: () => void; onDisconnect: (connection: ConnectedInstitution) => void; syncing: boolean }) {
  const [selectedAccountId, setSelectedAccountId] = useState<string | null>(null);
  const selectedAccount = accounts.find(account => account.id === selectedAccountId) ?? null;
  return <section className="ledger-widget accounts-page">{sandboxEnabled ? <><div className="accounts-toolbar"><span><p className="empty-copy">Connected Sandbox accounts can be refreshed without going through Link again.</p>{syncMessage ? <small className="sync-message">{syncMessage}</small> : null}</span><button className="primary-action" disabled={syncing || connections.length === 0} onClick={onSync}>{syncing ? "Syncing accounts…" : "Sync connected accounts"}</button></div>{connections.length > 0 ? <div className="connected-institutions">{connections.map(connection => <div key={connection.id}><span><strong>{connection.institutionName}</strong><small>{connection.accountCount} linked account{connection.accountCount === 1 ? "" : "s"} · {connection.environment}</small></span><button disabled={syncing} onClick={() => onDisconnect(connection)}>Disconnect</button></div>)}</div> : null}</> : null}<div className="account-grid">{accounts.map(account => <button type="button" className={`account-card ${selectedAccountId === account.id ? "selected" : ""}`} aria-pressed={selectedAccountId === account.id} onClick={() => setSelectedAccountId(current => current === account.id ? null : account.id)} key={account.id}><small>{account.accountType}</small><h3>{account.name}</h3><strong>{formatMoney(account.balanceCents)}</strong></button>)}<button className="connect-card" onClick={onAdd}><span>+</span><strong>Add local account</strong><small>Manual account or balance tracking</small></button>{sandboxEnabled ? <SandboxLinkButton onImported={() => window.location.reload()} /> : null}</div>{selectedAccount ? <section className="account-detail-panel" aria-label={`${selectedAccount.name} details`}><div><small>Selected account</small><h2>{selectedAccount.name}</h2><p>{selectedAccount.accountType}{selectedAccount.plaidSubtype ? ` · ${selectedAccount.plaidSubtype}` : ""}{selectedAccount.plaidMask ? ` · •••• ${selectedAccount.plaidMask}` : ""}</p></div><div className="account-detail-balance"><small>{selectedAccount.plaidRefreshedAt ? "Plaid reported balance" : "Current balance"}</small><strong>{formatMoney(selectedAccount.balanceCents)}</strong>{selectedAccount.plaidAvailableBalanceCents !== null && selectedAccount.plaidAvailableBalanceCents !== undefined ? <span>Available {formatMoney(selectedAccount.plaidAvailableBalanceCents)}</span> : null}</div><p className="account-detail-note">{selectedAccount.plaidRefreshedAt ? `Read-only balance from Plaid, refreshed ${new Date(selectedAccount.plaidRefreshedAt).toLocaleString()}. Use Sync connected accounts to refresh it.` : "This is a local account balance calculated from its opening balance and ledger activity."}</p></section> : null}</section>;
}
